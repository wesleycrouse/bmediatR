...
bmediatR includes built-in functions to help simulate data from a multiparental population (MPP) such as the Collaborative Cross (CC) or Diversity Outbred (DO) mice. Below, we randomly select a genetic locus `X` and simulate `M` and `Y` such that `X` explains 30% of the variance in `Y`, but does not influence `M`.
```{r, eval = F}
# set seed
set.seed(50)
...
### Apply Bayesian model selection with `bmediatR`
...
```{r}
# add tikz figure of DAG used to simulate data
```
...
The results of qtl mapping show that both `M` and `Y` have a QTL on chromosome 8.
```{r, out.width="100%"}
...
The allele effects for `M` and `Y` are strongly correlated.
...
`bmediatR` place the majority (0.76) of the posterior model probability on the co-local model, with the rest (0.24) on the partial mediation model. This demonstrates the method’s ability to distinguish between complete mediation and co-local relationships, even when allele effects are correlated.
...
QTL mapping shows that both M and Y have co-mapping QTLa on chromosome 6.
```{r, out.width="100%"}
...
```
BmediatR places the entire posterior probability on the complete mediation model, which aligns with our expectation.

## conclusion 

BmediatR excels when working with mpps…
